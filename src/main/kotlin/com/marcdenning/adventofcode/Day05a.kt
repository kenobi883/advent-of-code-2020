...
fun main(args: Array<String>) {
    Scanner(File(args[0])).use { scanner ->
        val maxSeatId = scanner.findAll("[BF]{7}[LR]{3}").map { matchResult ->
            val rowId = Day05a.extractRow(matchResult.group())
            val columnId = Day05a.extractColumn(matchResult.group())

            8 * Day05a.determineRow(rowId, 0, 0, NUMBER_OF_ROWS - 1) + Day05a.determineColumn(columnId, 0, 0, NUMBER_OF_COLUMNS - 1)
